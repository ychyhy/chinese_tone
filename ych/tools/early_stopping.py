...
class EarlyStopping:
    #
    def __init__(self, patience=7, verbose=False, ckpt_dir="./checkpoint.pt", metrics="loss"):
        """
    ...
    def __call__(self, val_loss, model):
        ...
        score = -val_loss if self.metrics == "loss" else val_loss
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            if self.metrics == 'loss':
                ...
                logger.info('Validation F1/acc increased ({:.6f} --> {:.6f}).  Saving model ...'.format(self.val_loss_min,
                                                                                                      val_loss))
        model_to_save = model.module if hasattr(model, 'module') else model
        torch.save(model_to_save.state_dict(), self.ckpt_dir)
